...
import 'buyer_group_detail_page.dart';

class BuyerGroupPage extends StatefulWidget {
  const BuyerGroupPage({super.key});

  @override
  State<BuyerGroupPage> createState() => _BuyerGroupPageState();
}

class _BuyerGroupPageState extends State<BuyerGroupPage> {
  final _searchController = TextEditingController();
  Timer? _debounce;
  List<BuyerGroupModel> _items = [];
  bool _loading = true;
  String? _error;
  UserModel? _user;
  ...
  bool _can(String action) =>
      ...
      _user?.permissions['buyer_group.${action.toLowerCase()}'] == true;
  ...
  Future<void> _load() async {
    ...
    final result = await BuyerGroupService.list(search: _searchController.text);
    if (!mounted) return;
    setState(() {
      _loading = false;
      _items = result.items;
  ...
  Future<void> _openForm([BuyerGroupModel? item]) async {
    final changed = await showDialog<bool>(
      context: context,
      builder: (_) => _BuyerGroupForm(item: item),
    );
    if (changed == true) _load();
  }

  Future<void> _delete(BuyerGroupModel item) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Hapus buyer group?'),
    ...
    final result = await BuyerGroupService.delete(item.id);
    if (!mounted) return;
...
class _BuyerGroupForm extends StatefulWidget {
  final BuyerGroupModel? item;
  const _BuyerGroupForm({this.item});

  @override
  State<_BuyerGroupForm> createState() => _BuyerGroupFormState();
}

class _BuyerGroupFormState extends State<_BuyerGroupForm> {
  ...
  Future<void> _save() async {
    ...
    final result = widget.item == null
        ? await BuyerGroupService.create(data)
        : await BuyerGroupService.update(widget.item!.id, data);
    if (!mounted) return;
